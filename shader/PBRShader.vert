...
void main() {
    ...
    N_in = normalize(normalMtx * vNormal);
    T_in = normalMtx * vTangent;
    T_in = normalize(T_in - dot(T_in, N_in) * N_in);   // Schmidt正交化
    vec3 B = cross(N_in, T_in);

    mat3 TBN_inverse    = transpose(mat3(T_in, B, N_in)); // ViewSpace -> TanSpace
